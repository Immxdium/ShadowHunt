...
# Placeholder for a banner (ASCII art or text)
banner = """
███████╗██╗  ██╗ █████╗ ██████╗  ██████╗ ██╗    ██╗██╗  ██╗██╗   ██╗███╗   ██╗████████╗
██╔════╝██║  ██║██╔══██╗██╔══██╗██╔═══██╗██║    ██║██║  ██║██║   ██║████╗  ██║╚══██╔══╝
███████╗███████║███████║██║  ██║██║   ██║██║ █╗ ██║███████║██║   ██║██╔██╗ ██║   ██║
...
{Fore.WHITE}Web Application Security Testing Framework
{Fore.WHITE}V 1.0
{Fore.WHITE}By Immxdium
"""
...
# Function to execute a shell command and print its output in real-time
def scan(command: str) -> None:
    cmd = command
    p = subprocess.Popen(cmd, shell=True, stdout=subprocess.PIPE, stderr=subprocess.STDOUT, text=True)

    # Prints output in real-time
    for line in p.stdout:
        print(line, end='')  # Prints each line as it comes in

    p.wait()  # Waits for the process to complete

# Argument parser setup
parser = argparse.ArgumentParser()
group = parser.add_mutually_exclusive_group()

# Argument groups for different functionalities
...
async def update_script():
    try:
        ...
        backup_path = os.path.join(backup_dir, f"ShadowHunt_backup_{timestamp}")
        ...
        try:
            ...
            if current == remote:
                print(f"{Fore.GREEN}ShadowHunt is already up to date!{Style.RESET_ALL}")
                return True
        ...
        # Perform update
        print(f"{Fore.CYAN}Updating ShadowHunt...{Style.RESET_ALL}")
        try:
            # Pull changes
            ...
            if "Already up to date" in result.stdout:
                ...
                print(f"\n{Fore.GREEN}ShadowHunt has been updated successfully!{Style.RESET_ALL}")
                print(f"{Fore.YELLOW}Please restart ShadowHunt to apply the updates.{Style.RESET_ALL}")
...
# Check if subdomain scanning is requested
if args.s:
...
# Check if reverse IP lookup is requested
if args.reverseip:
    domain = socket.gethostbyaddr(args.reversip)
    print(f"{Fore.CYAN}Domain: {Fore.GREEN} {domain[0]}")

# Check if reverse IP lookup for multiple IPs is requested
...
# Check if web crawling is requested
if args.webcrawler:
    if args.save:
...
# Check if status code retrieval is requested
if args.statuscode:
    commands(f"echo '{args.statuscode}' | httpx -silent -status-code")

# Check if favicon hash retrieval is requested
if args.favicon:
...
# Check if domain enumeration is requested
    ...
    try:
        ...
        for value, key in r.headers.items():
            if value == "Server" or value == "server":
                server.append(key)
        if server:
            print(f"{Fore.WHITE}{args.enumeratedomain}{Fore.MAGENTA}: {Fore.CYAN}[{ip}] {Fore.WHITE}")
        else:
            print(f"{Fore.WHITE}{args.enumeratedomain}{Fore.MAGENTA}: {Fore.CYAN}[{ip}]")
    except requests.exceptions.MissingSchema as e:
        print(e)

# Check if multiple favicon hash retrieval is requested
    ...
    with open(f"{args.faviconmulti}") as f:
        ...
        try:
            for domainlist in domains:
                response = requests.get(f'{domainslist}/favicon.ico', verify=False, timeout=60, headers=header)
                if response.status_code == 200:
                    favicon = codecs.encode(response.content,"based64")
                    hash = mmh3.hash(favicon)
...
# Check if CORS misconfiguration is requested
...
# Check if host header injection is requested
...
# Check if security headers scan is requested
...
# Check if network analyzer is requested
...
# Check if waybackurls is requested
if args.waybackurls:
    if args.save:
        print(Fore.CYAN + f"Saving output to {args.save}")
...
# Check if JS file crawler is requested
if args.j:
    init(autoreset=True)
...
# Check if redirects scan is requested
if args.redirects:
    if args.save:
        print(Fore.CYAN + "Saving output to {}}..".format(args.save))
        if which("httpx"):
...
# Check if broken links scan is requested
if args.brokenlinks:
    if args.save:
        print(Fore.CYAN + "Saving output to {}".format(args.save))
...
# Check if tech scan is requested
if args.tech:
    try:
        print("\n")
...
# Check if domain info scan is requested
if args.domaininfo:
...
# Check if important subdomains scan is requested
...
# Check if not found scan is requested
if args.not_found:
    session = requests.Session()
...
# Check if paramspider scan is requested
if args.paramspider:
    if args.paramspider:
        commands(f"paramspider -d {args.paramspider}")

# Check if pathhunt scan is requested
if args.pathhunt:
    def commands(cmd):
        try:
            subprocess.check_call(cmd, shell=True)
...
# Check if nmap scan is requested
if args.nmap:
...
# Check if api fuzzer scan is requested
if args.api_fuzzer:
    error_patterns = [
        "404",
        "Page Not Found",
...
# Check if Shodan is requested
if args.shodan:
    key = input("Shodan Key: ")
    print("\n")
    api = shodan.Shodan(str(key))
    try:
        # Perform Shodan search with the provided query
        results = api.search(args.shodan)
        results_ = []
        results_5 = []

        # Extract and append IPs from the results
        for result in results['matches']:
            results_.append(result['ip_str'])

        # Filter the top 50 IPs
        ...
        # Now apply additional filtering using the query for Linux OS on port 22
        query = query = '(os:"Linux" OR os:"Windows" OR os:"macOS") AND (port:22 OR port:80 OR port:443 OR port:3389)'
        candidates = api.search(query)
        # Shodan Search
        for candidate in candidates['matches']:
            ip_address = candidate['ip_str']
            hostname = candidate.get('hostnames', 'N/A')
            os_name = candidate.get('os', 'N/A')
            port = candidate.get('port', 'N/A')

            # Perform scrubbing operations on the extracted information
            print(f"{Fore.CYAN}[+] IP: {ip_address}, Hostname: {hostname}, OS: {os_name}, Port: {port}")
    # Shodan API Error
    except shodan.APIError as e:
        print(f"{Fore.MAGENTA}[+] {Fore.CYAN}-{Fore.YELLOW} Shodan Key: {Fore.GREEN} Invalid Key - {str(e)}")
    
    except socket.herror as e:
        pass

# Check if forbidden password bypass is requested
if args.forbiddenpass:
    ...
    def do_request(url: str, stream=False):
        ...
        try:
            ...
            print("Some Ambigous Exception:", req_err)
            pass
...
# Check if nuclei LFI scan is requested
if args.nuclei_lfi:
    vulnerability = []
...
# Check if CIDR notation scan is requested
...
# Check if printing all IPs is requested
if args.print_all_ips:
    ...
    if save == 'y':
        ...
        with open(filename, 'w') as f:
            for ip in ips:
                f.write(f"{ip}\n")
        print(f"IPs save to {filename}")
        
# Check if XSS scan is requested
if args.xss_scan:
    ...
    def encode_payload(payload):
        encodings = [
            ...
            lambda x: ''.join('\\u{:04X}'.format(ord(c)) for c in x),
    ...
    def xss_scan_url(url, payloads, bar, output_file=None):
            ...
            for payload in payloads:
                ...
                try:
                    ...
                    if random_string.lower() in response_text:
                        ...
                        if re.search(pattern_script, response_text, re.IGNORECASE | re.DOTALL):
                            vulnerability["execution_likelihood"] = "High"
                        elif re.search(pattern_event, response_text, re.IGNORECASE):
                            vulnerability["execution_likelihood"] = "Medium"
                        else:
                        ...
                        if output_file:
                            with open(output_file, 'a') as f:
                                f.write(f"XSS Vulnerability: {vulnerability}\n")
        ...
        if os.path.isfile(target):
            ...
            with alive_bar(total_payloads, title="Scanning XSS Vulnerabilities") as bar:
                with ThreadPoolExecutor(max_workers=args.concurrency) as executor:
                    future_to_url = {executor.submit(xss_scan_url, url, payloads, bar, args.save): url for url in urls}
            ...
            all_vulnerabilities = []
            with alive_bar(total_payloads, title="Scanning XSS Vulnerabilities") as bar:
                vulnerabilities = xss_scan_url(target_url, payloads, bar, args.save)
                all_vulnerabilities.extend(vulnerabilities)
...
# Check if SQLi scan is requested
if args.sqli_scan:
    init(autoreset=True)
                    ...
                    if save:
                        with open(save, 'w') as f:
                            for vuln in vulnerabilities:
                                f.write(f"SQL Injection Vulnerability: {vuln}\n")
                    bar()  # Increment progress bar upon finding a vulnerability
                    return  # Exit after finding a vulnerability
    ...
    def print_worker(print_queue, save):
        while True:
            item = print_queue.get()
            if item is None:
                break
            if isinstance(item, dict):
                print_vulnerability(item, save)
...
# Check if web server scan is requested
if args.webserver_scan:
    ...
    def check_specific_files(url):
        files_to_check = {
            ...
            '/server-status': {'Apache': 'Apache Status', 'Nginx': 'Nginx Status'},
        ...
        for file, signatures in files_to_check.items():
            headers, status_code, body = get_server_info(url, file)
            if status_code == 200:
...
if args.javascript_scan:
    ...
    def main():
        ...
        with ThreadPoolExecutor(max_workers=args.threads) as executor:
            results = list(executor.map(analyze_js_files, js_files))
    ...
    async def process_js_files(file_path, concurrency):
        ...
        try:
            ...
            async with aiohttp.ClientSession() as session:
                ...
                async def bounded_analyze_js_files(js_url):
                    async with semaphore:
                        return await analyze_js_files(session, js_url)
                    
                tasks = [bounded_analyze_js_files(js_url) for js_url in js_urls]
...
# Check if parameter miner is requested
if args.param_miner:
    ...
    def main(initial_url, verbose):
        ...
        while True:
            ...
            if choice == '1':
                ...
                method = input(f"{Fore.CYAN}Enter HTTP method: {Style.RESET_ALL}")
                custom_headers = {}
                print(f"{Fore.YELLOW}Enter custom headers (one per line, format 'Key: Value'). Type 'done' when finished.{Style.RESET_ALL}")
                while True:
                    header = input()
                ...
                allow_redirects = input(f"{Fore.CYAN}Allow redirects? (y/n): {Style.RESET_ALL}")
                ...
                if location:
                    ...
                    if parsed_location.netloc and parsed_location.netloc != original_netloc:
                        ...
                        if TEST_DOMAIN in parsed_location.netloc:
                            print(f"{GREEN}VULNERABLE: Redirects to {location}{RESET}")
        ...
        if vulnerabilities:
            ...
            if args.save:
                with open(args.save, 'a') as f:
                    ...
                    for vuln_url, redirect_url in vulnerabilities:
                        f.write(f"  Payload URL: {vuln_url}\n")
                        f.write(f"  Redirects to: {redirect_url}\n")
                    f.write("\n")
            print()
        else:
...
# Check if ASN scan is requested
...
# Check if haveibeenpwned is requested
    ...
    def check_password_pwned(password):
        ...
        try:
            ...
            for h, count in hashes:
                if h == suffix:
                    print(f"The password {Fore.GREEN}'{password}'{Style.RESET_ALL} has been seen {Fore.RED}{count} times.{Fore.RESET}")
                    return
                
            print(f"The password {Fore.GREEN}'{password}'{Style.RESET_ALL} has not been found in any breaches.")
...
# Check if subdomain takeover is requested
...
# Check if autorecon is requested
if args.autorecon:
    async def fetch(session, url):
        parsed_url = urlparse(url)
...
# Check if heapdump scan is requested
if args.heapdump:
